...
// 递归 构建文件树
const directoryTree = (path:string,options:Options,onEachFile?:OnEachCallback ,onEachDirectory?:OnEachCallback,depth?:boolean): Item | null=> {
		...
		if(!options.withChildren){
			if(!depth){
				item.children = dirData.map(child => 
					directoryTree(PATH.join(path, child), options, onEachFile, onEachDirectory, true)
					).filter(e => !!e) as Item[];
				item.size = item.children.reduce((prev:number, cur:Item) => prev + (cur.size?cur.size:0), 0);
				if (onEachDirectory) {
					onEachDirectory(item, PATH, stats);
				}
			}
		}else {
			item.children = dirData.map(child => directoryTree(PATH.join(path, child), options, onEachFile, onEachDirectory, false)).filter(e => !!e) as Item[];;
			// 读取all 的时会计算全部大小
			item.size = item.children.reduce((prev, cur) => prev +  (cur.size?cur.size:0), 0);
			if (onEachDirectory) {
				onEachDirectory(item, PATH, stats);
			}
		}
